...
getmetatable("").__mod = function(a, b)
  ...
  elseif type(b) == "table" then
    local success, result = pcall(string.format, a, unpack(b))
    if not success then
      error("Not enough arguments passed for string: " .. a)
    end

    return result
  else
    local success, result = pcall(string.format, a, b)
    if not success then
      error("Not enough arguments passed for string: " .. a)
    end
...
string.right = function (str, width)
  width = width or ha_settings.screenwidth
	str = tostring(str)
	return string.rep(" ", width - string.len(str)) .. str
end

string.center = function (str, width)
  width = width or ha_settings.screenwidth
	str = tostring(str)
	return string.rep(" ", (width / 2) - math.round(string.len(str) / 2)) .. str
end
...
string.wrap = function (str, limit, indent, indent1, tail)
	...
	limit = limit or ha_settings.screenwidth
	tail = tail or ""
...
string.jleft = function (str, amt, var)
  local use_str = var or str
	use_str = tostring(use_str)

	if not use_str then error("No string passed to string.jleft().") end
	if not amt or amt < 0 then amt = 0 end

  local len = strip_cecho_tokens(use_str):len()
	return str .. string.rep(" ", amt - len)
end

string.jright = function (str, amt, var)
  local use_str = var or str
	use_str = tostring(use_str)

	if not use_str then error("No string passed to string.jright().") end
	if not amt or amt < 0 then amt = 0 end
...
string.jcenter = function (str, amt, var)
  local use_str = var or str
  use_str = tostring(use_str)

  if not use_str then error("No string passed to string.jcenter().") end
...
string.zeropad = function(str, z_length)
  str = tostring(str)
  if not z_length or type(z_length) ~= "number" then
    error("string.zeropad(): Invalid z_length passed. Requires a number.")
    return
  end

  return "%s%s" % {string.rep("0", z_length - str:len()), str}
end

string.rpc = function(str, pos, r)
  return "%s%s%s" % {str:sub(1, pos - 1), r, str:sub(pos + 1)}
end

string.inc = function(str, pos, c)
  return "%s%s%s" % {str:sub(1, pos - 1), c, str:sub(pos)}
end
...
function table.random(t)
  assert(t, "table.random() expected a table, got nil.")
